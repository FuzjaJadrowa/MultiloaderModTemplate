...
/**
 * Preprocessor that handles conditional compilation based on the active target version.
 * 
 * Directives:
 * - `//? if >=1.21.1 {`
 * - `//? elif <1.21 {`
 * - `//? else {`
 * - `//? }`
 * - `//? if <1.21.1` (applies to the immediate next line only)
 * 
 * Content inside block comments `/*` and `*/` inside active branches will be uncommented.
 */
object Preprocessor {
    ...
    /**
     * Define custom replacement mappings here to adapt source symbols dynamically
     * across different Minecraft versions without duplication.
     */
    private fun applyReplacements(line: String, version: String): String {
        var result = line
        
        // Example: Handle Identifier vs ResourceLocation mapping in 1.21
